      ...
      <div class="phone-input">
        ...
        <div class="input-box">
          ...
          <a
            ...
            @click="getCodeAction()"
          >{{codeButtonText}}</a>
          <a href="javascript:;" v-else class="get-code">
            <em>{{codeButtonText}}</em>
          </a>
        </div>
      </div>
      <div class="phone-input">
  ...
  methods: {
    ...
    getJsonData() {
      this.$http.get("../../static/user.json").then(res => {
        this.number = res.data.phoneList.verifyCode;
        alert("您的验证码是：" + this.number);
      });
    },
    // 获取验证码
    getCodeAction() {
    ...
    validate() {
      ...
      if (this.number === this.verifyCode) {
        this.getUserInfo();
      } else {
        alert("验证码错误");
      }
